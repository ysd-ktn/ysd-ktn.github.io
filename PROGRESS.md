...
### Phase 7-2 前の先行修正 ✅ (2026.05.06)

Phase 7-2 着手前に以下 2件を修正した。

- ✅ **Cursor 座標のモバイル非表示**
  - `Cursor.astro` の `@media (hover: none)` だけでは一部端末で `.cur-meta` (座標 `XXXX, YYYY` を含む) が画面左上に残って見える問題
  - `@media (hover: none), (max-width: 720px)` に変更し、プロジェクト共通ブレイクポイント 720px 以下でも確実に `.cursor` / `.cur-meta` を `display: none` に
  - **CSSカスケード落とし穴**: メディアクエリを `<style>` ブロックの先頭に置いていたため、後ろの `.cur-meta { display: flex }` に上書きされていた。同詳細度は後ろ勝ちなので **メディアクエリは必ずスタイルブロック末尾に** 置く
- ✅ **TIMELINE の EVENT / EXHIBITION エントリを [01.3/EVENTS] に切り分け**
  - `About.astro` で `timeline` を `tag` の有無でフィルタリング
  - `tag` なし (セガ / マネーフォワード / 京都市芸大) → `[01.2 / TIMELINE]` に残す
  - `tag` あり (ClusterGAMEJAM EVENT · 京都市芸バーチャル展 EXHIBITION) → `[01.3 / EVENTS]` に表示
  - `eventsTimeline.length > 0` の条件付きレンダリングなので、将来エントリが 0件になっても安全
  - セクション見出しは `EVENTS` (仮称。リネーム時は About.astro の `index}.3 / EVENTS` を変更するだけ)
  - `sec-head` の meta を `'PROFILE + TIMELINE + EVENTS'` に更新
  - 既存の `.timeline + .sub-head { margin-top: 56px }` セレクタが TIMELINE → EVENTS の間の余白もカバー済み

### Phase 7-2 Header モバイル挙動 ✅ (2026.05.07 完了)

**DoD**: タッチデバイスで Header のナビが操作できる状態。

#### 採用案: D-1 常時表示ナビバー

- モバイル (≤720px) ではピルを非表示にして、全幅の横置きナビバー (44px・1段) を常時表示
- ロゴ行なし — ナビ 4 項目 (00 INDEX / 01 ABOUT / 02 WRITING / 03 CONTACT) のみ
- デスクトップのピル hover 挙動は既存のまま維持
- アクティブ表示: テキスト accent 色 + `box-shadow: inset 0 -2px 0 var(--accent)` ボトムインジケーター
- `scroll-margin-top: 52px` を `#index / #about / #writing / #contact` に追加 (固定ナビバーがアンカーリンクを隠す問題の対処)
- `main` の mobile padding-top を 64px → 68px に調整 (ナビバー 44px + 余白 24px)
- `sec-head .meta span:nth-child(2)` をモバイルで `display: none` — "PROFILE + TIMELINE + EVENTS" 等の中段テキストが 390px 幅ではみ出す問題を解消。セクション名と版数だけ残す

#### スクロール連動アクティブ表示の実装と落とし穴

当初 `IntersectionObserver` で実装したが、2 つの問題があり `scroll + getBoundingClientRect()` 方式に変更した。

- **問題 1**: スムーススクロールのアニメーション中に途中セクションが検出帯に入り、アクティブが誤発火して噛み合わないように見えた
- **問題 2**: `scroll-margin-top: 52px` の影響で `getBoundingClientRect().top` が `52.X px` (浮動小数点) になり `<= 52` の条件が false になって最終セクションが検出されなかった

**最終実装**:
- `NAV_H = 60` (ナビバー 44px + scroll-margin 8px + 浮動小数点バッファ 8px) を閾値に設定
- 各セクションの `getBoundingClientRect().top <= NAV_H` を満たす最後のセクションをアクティブに
- ページ下端到達時 (`innerHeight + scrollY >= body.offsetHeight - 8`) は CONTACT を強制アクティブ
- ナビリンクのタップ時は即座にアクティブ化 (スクロールアニメーション中のズレ防止)

#### Phase 7-2 で作成したモック (比較検討用、`_mockups/` に保存)

- `option-mobile-header-A-hamburger.html` — フルスクリーンオーバーレイ
- `option-mobile-header-B-bottom-drawer.html` — ボトムドロワー
- `option-mobile-header-C-vertical.html` — ドロップダウン縦展開
- `option-mobile-header-D-exposed-nav.html` — 常時露出ナビ (横・縦 2 バリアント) ← **採用**
- `option-mobile-header-E-top-sheet.html` — トップシート

#### Phase 7-2 のコミット (push 未済 — Phase 7-1 の 15 コミットと合わせて push 推奨)

```
Add persistent mobile nav bar with scroll-spy
```
...
### Phase 8-1 favicon / OGP / meta タグ整備 ✅ (2026.05.10 完了)

- ✅ **favicon アセット一式** を `public/` 直下に配置
  - ユー制作: `favicon.svg` / `favicon.png` (512×512, 変換元) / `apple-touch-icon.png` / `og-image.png`
  - マロ生成: `favicon.ico` (16/32/48px 多サイズ ICO) / `favicon-96x96.png`
  - `favicon.png` は変換元につき削除して可
- ✅ **`Layout.astro` meta タグ全面整備**
  - Primary: `<title>` / `<meta name="description">` / `<link rel="canonical">`
  - Favicon: `favicon.ico` / `favicon.svg` / `favicon-96x96.png` / `apple-touch-icon` の `<link>` を追加
  - OGP: `og:type` / `og:url` / `og:title` / `og:description` / `og:image` / `og:locale`
  - Twitter Card: `summary_large_image` / `@ysd_ktn` ほか
- ✅ **`@astrojs/sitemap` 導入** → ビルド時に `sitemap-index.xml` を自動生成
- ✅ **`public/robots.txt`** 新規作成 (全クローラー許可 + sitemap URL 明示)
- ✅ **テキスト全面見直し**
  - `profile.json` の `roleHero` を `UI/UX DESIGNER · COMPLEXITY DISSOLVED BY DESIGN` に変更 (場所・受付状況を廃止しフィロソフィーへ)
  - `profile.json` の座標を川崎 (35.5308°N · 139.6968°E) → 東京 (35.6812°N · 139.7671°E) に修正
  - OGP description を `UI/UX Designer specializing in building design systems and visual design.` に

### Phase 8-2 Lighthouse + アクセシビリティ確認・修正 ✅ (2026.05.10 完了)

#### Lighthouse スコア (修正前 / 本番サイト測定)
...
- Accessibility 95: `--dim: #6b6b6b` のコントラスト不足 (3.71:1、要 4.5:1) のみ
- Performance 87: FCP/LCP が 3.2s (Google Fonts のレンダーブロッキングが原因)

#### 実施した修正

- ✅ **`--dim` カラー修正** (`global.css`)
  - `#6b6b6b` (3.71:1) → `#7e7e7e` (約 4.9:1) に変更
  - Hero `.index` / `.role`、About `dt` / timeline `.date`、Writing `excerpt` / meta、Contact `.label`、Footer status bar など全コンポーネントの `--dim` テキストが WCAG AA 準拠に
- ✅ **Google Fonts 非同期ロード** (`Layout.astro`)
  - `<link rel="stylesheet">` → `media="print" onload="this.media='all'"` パターンに変更
  - `<link rel="preload" as="style">` と `<noscript>` フォールバックを追加
  - レンダーブロッキングを解消し FCP/LCP 改善 (目標 95+)
- ✅ **`:focus-visible` スタイル追加** (`global.css`)
  - `outline: 2px solid var(--accent); outline-offset: 3px` でキーボードフォーカスを視覚化
- ✅ **デスクトップ nav の keyboard trap 解消** (`Header.astro`)
  - collapsed 時に `visibility: hidden` を追加 (Tab キーが非表示リンクに入らないよう)
  - hover 時は `visibility: visible` に即時切替 (delay: 0s)、非 hover 時は fade-out 後に hidden (delay: 0.4s)
- ✅ **デスクトップ nav に `aria-label` 追加** (`Header.astro`)
  - `<nav aria-label="Main navigation">` でモバイル nav と区別
...
### Phase 8-3 Google Analytics GA4 導入 ✅ (2026.05.10 完了)

- ✅ GA4 プロパティ作成、ウェブストリーム設定 (ysd-ktn.github.io)
- ✅ `Layout.astro` に gtag.js スクリプトを追加 (Measurement ID: G-SQJ7LLGG1L)
  - `<script async src="https://www.googletagmanager.com/gtag/js?id=G-SQJ7LLGG1L">` + 初期化スクリプト
  - フォント読み込みの `<noscript>` の直後、`</head>` の前に配置
- ✅ push → GitHub Actions デプロイ完了・GA リアルタイムで計測確認済み

---

## Phase 8-4 — [01.2 / TIMELINE] トグル詳細 ✅ (2026.05.10 完了)

- ✅ `content.config.ts`: `timelineSchema` に `description` (string, optional) と `links` (array of `{label, url}`, optional) を追加
- ✅ `timeline.json`: キャリア3エントリ (セガ / マネーフォワード / 京都市芸) に仮文言 `description` を追加
- ✅ `About.astro`:
  - `TimelineEntry` interface に `id?`, `description?`, `links?` を追加
  - careerTimeline の各行に `[+]` トグルボタン + 詳細行 (`timeline-detail`) を追加
  - CSS: `.tl-toggle` (ボタン) / `.timeline-detail` (デフォルト非表示) / `.detail-body` / `.detail-text` / `.detail-link` を追加
  - モバイル: `.timeline-detail.is-open { display: block }` を追加
  - Script: クリックで `.is-open` クラスのトグル + `aria-expanded` + アイコン `+` / `−` 切替
- `description` フィールドは仮文言入り (`※ 仮文言 — ここを書き換えてください`) — 本文は `timeline.json` を直接編集して差し替え
- `links` フィールドは全エントリ未設定 — リンクを追加したい場合は `{ "label": "...", "url": "https://..." }` 形式で追加
- [01.3 / EVENTS] はトグルなし (将来追加したい場合は eventsTimeline.map 側も同様に対応する)
- ✅ 行全体をクリック範囲に拡張 (`tr.has-detail` にリスナー移動、ボタンクリックは tr にバブリング)
- ✅ ホバースタイル変更: `[+]` 枠線を廃止してフラットなアイコンに、行の `border-bottom` がアクセントカラー・solid に変化
- ✅ `data-cursor-chip="DETAILS"` を button から `tr` に移動 (行ホバー時にチップ表示)
- ✅ 本文を実文言に差し替え・卒業論文要旨リンクを追加 (timeline.json)
- ビルド検証済み (`astro build` エラーなし)
...
- **ライトモード** ❌ — ダーク専用に確定 (2026.05.10)
  - `#00f0bd` はダーク背景前提の発光色で白背景では WCAG AA 非準拠 (1.5:1)
  - 補色 (#EE0022) での代替も検討したが、サイトの世界観がダーク固定で完結しているため不要と判断
  - WORKS 復活・コンテンツ量増加後に改めて検討する余地はあるが、当面は作らない
- **問い合わせフォーム vs mailto** ❌ — mailto のまま確定 (2026.05.10)
