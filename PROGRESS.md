...
最終更新: 2026.05.05 (Phase 6.5 完了 ✅ — Writing 記事サムネを実画像 (.webp 3枚) に差し替えてコミット&push 済、 本番反映済。 残りの画像系 (favicon / OG image) は Phase 8 で対応。 次は **Phase 7 (レスポンシブ対応)** に着手)
...
- Writing サムネ画像の `grayscale(0.85)` を **モバイル (`@media (hover: none)` 系) では解除してフルカラー表示**。 デスクトップは hover で grayscale → カラーに点灯する演出だが、 タッチデバイスは hover が無いので最初からフルカラーが妥当 (Phase 6.5 で実画像差し替え時に発生した宿題)
