...
最終更新: 2026.05.01 (Phase 4 進行中、Footer.astro 完了 — 残り Cursor.astro)
...
- ✅ **Phase 4-3** `src/components/Contact.astro` (key/value リスト)
  ...
  - 'copy' は `<button class="ext copy-btn">→ COPY</button>` で出力、 click で `navigator.clipboard.writeText(value)` → 1.5秒だけラベルを `→ COPIED ✓` + アクセント色に切替
...
- 本物のデータを投入する対象:
  - PROFILE: 本名表記 / 役職 / 拠点 / FOCUS / TOOLS / WRITING 自己紹介 / STATUS
  - TIMELINE: 実際の経歴 + 展覧会
  - WRITING: 実 note 記事 (タイトル / 日付 / URL / likes / `totalCount`) + `noteProfileUrl`
  - CONTACT: 実 email / X / Instagram / GitHub のリンク
