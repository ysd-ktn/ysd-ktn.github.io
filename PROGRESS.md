...
- `src/components/About.astro` (PROFILE + TIMELINE)
  - dl/dt/dd の spec シート (NAME / ROLE / LOCATION / FOCUS / TOOLS / WRITING / STATUS)
  - timeline テーブル (EXHIBITION タグ + PRESENT のアクセント色)
- `src/components/Writing.astro` (3カードグリッド + MORE リンク)
  - meta 行 ([ idx ] / 日付 / ♡ likes) + 16:9 サムネ + タイトル + 概要 + NOTE → READ
  - 下部に `VIEW ALL ARTICLES · +N MORE → NOTE PROFILE` の CTA
- `src/components/Contact.astro` (key/value リスト)
  - email (mailto + COPY) / X / Instagram / GitHub
- `src/components/Footer.astro` (画面下固定ステータスバー)
  - ● ONLINE · BUILD / CURSOR · X,Y / UTC HH:MM:SS / VIEWPORT · W×H
  - ● は 1.4秒間隔で点滅 (アクセント色)
  - UTC 時刻と viewport は client-side JS で更新
- `src/components/Cursor.astro` (Figma 風 SVG カーソル + チップ + 座標)
...
完了後は `npm run dev` でモック完全再現を確認 → コミット → push。
