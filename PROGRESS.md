...
- ✅ `.github/workflows/deploy.yml` を作成・修正完了
  - Build ジョブ: Checkout@v5 → Node.js 22 → `npm install` → `astro build` → `actions/upload-pages-artifact@v3`
  - Deploy ジョブ: `actions/deploy-pages@v4` (GitHub Pages 公式アクション)
  - environment: name: github-pages 設定済み
  - permissions: pages: write, id-token: write 設定済み
  - Node.js v22 指定（Astro 要件）

- ✅ gh-pages ブランチ作成済み
  - ユーが手動で初期化（orphan branch で placeholder 配置）
  - GitHub に push 完了

- ✅ GitHub Pages Settings 変更済み
  - Source を `gh-pages branch` に指定済み

### 次セッションでやること

**新セッション開始時の作業フロー：**

1. GitHub Settings → Environments → `github-pages` environment を確認
   - 存在しなければ、GitHub Actions が自動作成する（最初のワークフロー実行時）
   - Deployment branches が「All branches」になってることを確認（保護ルールなし）

2. `git push origin master` または GitHub Web から最新ワークフロー実行
   - Build ジョブ: ✅ 成功するはず（Node 22 + Astro build 成功）
   - Deploy ジョブ: environment 設定があるため、github-pages environment に deploy
   - 🟢 両方成功すると `https://ysd-ktn.github.io` で公開される

3. 実機確認
   - `https://ysd-ktn.github.io` をブラウザで開く
   - ポートフォリオが表示されることを確認

### 既知の注意点

- peaceiris/actions-gh-pages は GitHub の新しい OIDC-based deployment に非対応のため、GitHub Pages 公式アクション `actions/deploy-pages@v4` に変更した
- actions/upload-pages-artifact@v3 と actions/deploy-pages@v4 はセットで使用（互換性重要）
- environment: github-pages は deploy ジョブで必須（environment protection rules で管理される）
