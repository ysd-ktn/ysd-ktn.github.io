 ...
 *
 *  見えない文字の除去を先にやるのが要点。
 *  `✍️` は「✍ (U+270D)」と「異体字セレクタ (U+FE0F)」の2文字でできており、
 *  slug 生成では目に見える ✍ だけが落ちて U+FE0F が残る。
 *  すると先頭が「U+FE0F + ハイフン」になり、 ハイフンだけを見る掃除では
 *  先頭判定に引っかからず `#️-練習方法` のような id ができてしまう。
 *  絵文字によって残るものと残らないものがあるので (🎨 や 🌱 は残らない)、
 *  見た目には気づけない。
 */
export function cleanId(raw: string): string {
  return raw
    // 異体字セレクタ / ゼロ幅文字を落とす。
    // ソースに直接書くと自分自身が見えず修正もできないので、必ず番号で書く。
    //   U+FE00-FE0F 異体字セレクタ (絵文字の後ろに付く見えない1文字)
    //   U+200B-200D ゼロ幅スペース / ノンジョイナ / ジョイナ
    //   U+2060      ワードジョイナ
    //   U+FEFF      BOM
    .replace(/[\uFE00-\uFE0F\u200B-\u200D\u2060\uFEFF]/g, '')
